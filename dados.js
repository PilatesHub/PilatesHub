let dados = [
    {
        nome: "Roll Up",
        descricao: "Posição: Deitado de costas com as pernas estendidas e os braços acima da cabeça. Execução: Enrole o corpo para cima, vértebra por vértebra, até sentar-se, e depois role para trás até a posição inicial.",
        nivel: "Nível: Intermediário",
        local: "Local: Solo",
        musculosTrabalhados: ["Músculos: Abdômen", " paravertebrais", " flexores do quadril"],
        imagem: "roll_up.jpg",
        link: "FZNwIJ03fhQ",
        video: "roll_up.mp4"
      },
      {
        nome: "Single Straight Leg Stretch",
        descricao: "Exercício similar ao Single Leg Stretch, mas com a perna esticada em vez de flexionada.",
        nivel: "Nível: Intermediário",
        local: "Local: Solo",
        musculosTrabalhados: ["Músculos: abdominais", " oblíquos", " flexores do quadril"],
        imagem: "single_straight_leg_stretch.jpg",
        link: "uIdc2jmlD7k",
        video: "single_straight_leg_stretch.mp4"
      },
      {
        nome: "Double Straight Leg Stretch",
        descricao: "Variação do Single Straight Leg Stretch, onde ambas as pernas são esticadas.",
        nivel: "Nível: Avançado",
        local: "Local: Solo",
        musculosTrabalhados: ["Músculos: abdominais", " oblíquos", " flexores do quadril"],
        imagem: "double_straight_leg_stretch.jpg",
        link: "MRzMheJcPJ8",
        video: "double_straight_leg_stretch.mp4"
      },
      {
        nome: "Shell Stretch",
        descricao: "Exercício que alonga a coluna e os músculos posteriores das pernas. Envolve curvar o corpo em forma de concha.",
        nivel: "Nível: Básico",
        local: "Local: Solo",
        musculosTrabalhados: ["Músculos: isquiotibiais", " glúteos", " eretores da espinha"],
        imagem: "shell_stretch.jpg",
        link: "7zsKsz4uQe4",
        video: "shell_stretch.mp4"
      },
      {
        nome: "The Hundred",
        descricao: "Posição: Deitado de costas com as pernas elevadas em posição de mesa (joelhos dobrados a 90 graus) ou estendidas, braços ao lado do corpo. Execução: Eleve a cabeça e os ombros do solo, mantendo o core engajado, e bombeie os braços para cima e para baixo enquanto inspira por 5 contagens e expira por 5 contagens, repetindo até 100.",
        nivel: "Nível: Básico",
        local: "Local: Solo",
        musculosTrabalhados: ["Músculos: Abdômen", " músculos respiratórios", " quadríceps"],
        imagem: "the_hundred.jpg",
        link: "On2BPeK4q2Y",
        video: "the_hundred.mp4"
        },
        {
          nome: "Half Roll Down",
          descricao: "Posição: Sentado com as pernas dobradas, pés no chão, braços estendidos à frente. Execução: Enrole a coluna lentamente para trás, mantendo o abdômen ativado, até a metade do caminho, e depois retorne à posição inicial.",
          nivel: "Nível: Básico",
          local: "Local: Solo",
          musculosTrabalhados: ["Músculos: Abdômen", " paravertebrais", " quadríceps"],
          imagem: "half_roll_down.jpg",
          link: "zTqCd39YcbE",
          video: "half_roll_down.mp4"
          },
          {
          nome: "Pelvic Curl",
          descricao: "Posição: Deitado de costas com os joelhos dobrados e pés no chão, braços ao lado do corpo. Execução: Eleve a pélvis do chão, vértebra por vértebra, até formar uma linha reta dos ombros aos joelhos. Retorne lentamente à posição inicial.",
          nivel: "Nível: Básico",
          local: "Local: Solo",
          musculosTrabalhados: ["Músculos: Glúteos", " isquiotibiais", " abdômen", " músculos da coluna"],
          imagem: "pelvic_curl.jpg",
          link: "gE018HYqqEM",
          video: "pelvic_curl.mp4"
          },
          {
          nome: "Single Leg Stretch",
          descricao: "Posição: Deitado de costas com a cabeça e os ombros elevados, uma perna estendida e a outra dobrada. Execução: Alterne as pernas, trazendo uma em direção ao peito enquanto a outra se estende.",
          nivel: "Nível: Básico",
          local: "Local: Solo",
          musculosTrabalhados: ["Músculos: Abdômen", " flexores do quadril", " quadríceps"],
          imagem: "single_leg_stretch.jpg",
          link: "-5-NRRq6q1M",
          video: "single_leg_stretch.mp4"
          },
          {
          nome: "Double Leg Stretch",
          descricao: "Posição: Deitado de costas com as pernas dobradas em direção ao peito e as mãos segurando as canelas. Execução: Estenda simultaneamente os braços acima da cabeça e as pernas à frente, depois retorne à posição inicial.",
          nivel: "Nível: Básico",
          local: "Local: Solo",
          musculosTrabalhados: ["Músculos: Abdômen", " flexores do quadril", " quadríceps"],
          imagem: "double_leg_stretch.jpg",
          link: "N-jZas9tMSU&t=33s",
          video: "double_leg_stretch.mp4"
          },
          {
          nome: "Scissors",
          descricao: "Posição: Deitado de costas com a cabeça e os ombros elevados, pernas estendidas para cima. Execução: Abaixe uma perna em direção ao chão enquanto puxa a outra em direção ao peito, alternando as pernas rapidamente.",
          nivel: "Nível: Intermediário",
          local: "Local: Solo",
          musculosTrabalhados: ["Músculos: Abdômen", " flexores do quadril", " quadríceps", "isquiotibiais"],
          imagem: "scissors.jpg",
          link: "CO9_19vfCf4",
          video: "scissors.mp4"
          },
          {
          nome: "Leg Circles",
          descricao: "Posição: Deitado de costas com uma perna estendida no chão e a outra elevada. Execução: Faça círculos controlados com a perna elevada, mantendo o quadril estável. Alterne o sentido dos círculos.",
          nivel: "Nível: Básico",
          local: "Local: Solo",
          musculosTrabalhados: ["Músculos: Abdômen", " flexores do quadril", " adutores e abdutores do quadril"],
          imagem: "leg_circles.jpg",
          link: "RZqtVL6K8DM",
          video: "leg_circles.mp4"
          },
          {
          nome: "Rolling Like a Ball",
          descricao: "Posição: Sentado, com os joelhos dobrados e as mãos segurando as canelas, equilibrando-se sobre o cóccix. Execução: Role para trás até os ombros e, em seguida, retorne à posição inicial.",
          nivel: "Nível: Básico",
          local: "Local: Solo",
          musculosTrabalhados: ["Músculos: Abdômen", " paravertebrais", " flexores do quadril"],
          imagem: "rolling_like_a_ball.jpg",
          link: "elkcXFPyaW8",
          video: "rolling_like_a_ball.mp4"
          },
          {
          nome: "Spine Stretch Forward",
          descricao: "Posição: Sentado com as pernas estendidas e abertas na largura dos ombros, braços estendidos à frente. Execução: Inspire e alongue a coluna para cima, depois expire e curve a coluna para frente, como se estivesse enrolando vértebra por vértebra.",
          nivel: "Nível: Básico",
          local: "Local: Solo",
          musculosTrabalhados: ["Músculos: Paravertebrais", " abdômen", " isquiotibiais"],
          imagem: "spine_stretch_forward.jpg",
          link: "yso-Y3Ik2BM",
          video: "spine_stretch_forward.mp4"
          },
          {
          nome: "Single Leg Kick",
          descricao: "Posição: Deitado de bruços, apoiado nos antebraços. Execução: Flexione uma perna e dê um chute rápido, mantendo o quadril estável, depois troque a perna.",
          nivel: "Nível: Básico",
          local: "Local: Solo",
          musculosTrabalhados: ["Músculos: Isquiotibiais", " glúteos", " abdômen"],
          imagem: "single_leg_kick.jpg",
          link: "X79gMUknrVs",
          video: "single_leg_kick.mp4"
          },
          {
          nome: "Side Lying Leg Lifts",
          descricao: "Posição: Deitado de lado, com o corpo alinhado e uma perna sobre a outra. Execução: Levante a perna de cima para cima e para baixo, mantendo o corpo estável.",
          nivel: "Nível: Básico",
          local: "Local: Solo",
          musculosTrabalhados: ["Músculos: Abdutores do quadril", " glúteos"],
          imagem: "side_lying_leg_lifts.jpg",
          link: "sAamLbKW-Zc",
          video: "side_lying_leg_lifts.mp4"
          },
          {
          nome: "Swan Prep",
          descricao: "Posição: Deitado de bruços com as mãos próximas aos ombros. Execução: Eleve o tronco do chão, usando os músculos das costas e apoiando-se ligeiramente nas mãos.",
          nivel: "Nível: Básico",
          local: "Local: Solo",
          musculosTrabalhados: ["Paravertebrais", "glúteos", "abdômen"],
          imagem: "swan_prep.jpg",
          link: "Ab4eTe2R8z4",
          video: "swan_prep.mp4"
          },
          {
          nome: "Breast Stroke Prep",
          descricao: "Posição: Deitado de bruços com os braços estendidos ao lado do corpo. Execução: Eleve o tronco e os braços simultaneamente, imitando o movimento de nado.",
          nivel: "Nível: Básico",
          local: "Local: Solo",
          musculosTrabalhados: ["Paravertebrais", "ombros", "glúteos"],
          imagem: "breast_stroke_prep.jpg",
          link: "AwhZnmJZB_Q",
          video: "breast_stroke_prep.mp4"
          },
          {
          nome: "Shoulder Bridge",
          descricao: "Posição: Deitado de costas, com os joelhos dobrados e pés no chão. Execução: Eleve a pélvis, formando uma linha reta dos ombros aos joelhos.",
          nivel: "Nível: Básico",
          local: "Local: Solo",
          musculosTrabalhados: ["Músculos: Glúteos", " isquiotibiais", " paravertebrais"],
          imagem: "shoulder_bridge.jpg",
          link: "jCSfXZToPCI",
          video: "shoulder_bridge.mp4"
          },
          {
          nome: "Cat Stretch",
          descricao: "Posição: Em quatro apoios, com os joelhos alinhados com o quadril e mãos alinhadas com os ombros. Execução: Enrole a coluna para cima (arredondando), depois estenda-a para baixo (arqueando).",
          nivel: "Nível: Básico",
          local: "Local: Solo",
          musculosTrabalhados: ["Músculos: Paravertebrais", " abdômen", " músculos profundos da coluna"],
          imagem: "cat_stretch.jpg",
          link: "l0-VbgSvdhs",
          video: "cat_stretch.mp4"
          },
          {
            nome: "Criss-Cross",
            descricao: "Posição: Deitado de costas com as pernas dobradas e as mãos atrás da cabeça. Execução: Leve o cotovelo em direção ao joelho oposto, alternando os lados.",
            nivel: "Nível: Intermediário",
            local: "Local: Solo",
            musculosTrabalhados: ["Músculos: Oblíquos", " abdômen", " flexores do quadril"],
            imagem: "criss_cross.jpg",
            link: "gzaCxDVQL90",
            video: "criss_cross.mp4"
            },
            {
            nome: "Open Leg Rocker",
            descricao: "Posição: Sentado com as pernas estendidas em V e as mãos segurando as pernas próximas aos tornozelos. Execução: Enrole a coluna para trás até os ombros e volte à posição inicial, mantendo as pernas estendidas e o controle do movimento.",
            nivel: "Nível: Intermediário",
            local: "Local: Solo",
            musculosTrabalhados: ["Músculos: Abdômen", " paravertebrais", " flexores do quadril"],
            imagem: "open_leg_rocker.jpg",
            link: "ZbtEw_pcPp4",
            video: "open_leg_rocker.mp4"
            },
            {
            nome: "Double Leg Kick",
            descricao: "Posição: Deitado de bruços, mãos entrelaçadas atrás das costas e pernas estendidas. Execução: Dobre ambas as pernas e dê dois chutes rápidos, em seguida estenda as pernas enquanto eleva o tronco e os braços para trás.",
            nivel: "Nível: Intermediário",
            local: "Local: Solo",
            musculosTrabalhados: ["Músculos: Isquiotibiais", " glúteos", " paravertebrais"],
            imagem: "double_leg_kick.jpg",
            link: "eQgIt5Ikb1g",
            video: "double_leg_kick.mp4"
            },
            {
            nome: "Shoulder Bridge",
            descricao: "Posição: Deitado de costas, com os joelhos dobrados e pés no chão. Execução: Eleve a pélvis e depois uma perna, mantendo o quadril estável. Abaixe a perna e repita com a outra.",
            nivel: "Nível: Intermediário",
            local: "Local: Solo",
            musculosTrabalhados: ["Músculos: Glúteos", " isquiotibiais", " paravertebrais"],
            imagem: "shoulder_bridge.jpg",
            link: "jCSfXZToPCI&t=17s",
            video: "shoulder_bridge.mp4"
            },
            {
            nome: "Side Kick Series",
            descricao: "Posição: Deitado de lado, corpo alinhado e uma perna sobre a outra. Execução: Movimente a perna de cima para frente e para trás, mantendo o controle. Realize variações, como chutes para cima e círculos.",
            nivel: "Nível: Intermediário",
            local: "Local: Solo",
            musculosTrabalhados: ["Músculos: Abdutores e adutores do quadril", " glúteos"],
            imagem: "side_kick_series.jpg",
            link: "nG9JfDHJJlY",
            video: "side_kick_series.mp4"
            },
            {
            nome: "Swimming",
            descricao: "Posição: Deitado de bruços com os braços estendidos à frente. Execução: Levante os braços e as pernas do chão e mova-os alternadamente, simulando um movimento de natação.",
            nivel: "Nível: Intermediário",
            local: "Local: Solo",
            musculosTrabalhados: ["Músculos: Paravertebrais", " glúteos", " ombros"],
            imagem: "swimming.jpg",
            link: "bY6ZyiO_7ek",
            video: "swimming.mp4"
            },
            {
            nome: "Seal",
            descricao: "Posição: Sentado com os pés juntos e as mãos segurando os tornozelos, equilibrando-se sobre o cóccix. Execução: Role para trás até os ombros e, ao voltar, bata os pés três vezes como uma foca.",
            nivel: "Nível: Intermediário",
            local: "Local: Solo",
            musculosTrabalhados: ["Músculos: Abdômen", " paravertebrais", " flexores do quadril"],
            imagem: "seal.jpg",
            link: "G5zO03AJlwU",
            video: "seal.mp4"
            },
            {
            nome: "Teaser",
            descricao: "Posição: Deitado de costas com as pernas estendidas e os braços acima da cabeça. Execução: Levante as pernas e o tronco simultaneamente, formando um V, mantendo os braços estendidos na direção das pernas.",
            nivel: "Nível: Intermediário",
            local: "Local: Solo",
            musculosTrabalhados: ["Músculos: Abdômen", " flexores do quadril", " paravertebrais"],
            imagem: "teaser.jpg",
            link: "UJ5gZQSqlXo",
            video: "teaser.mp4"
            },
            {
            nome: "Leg Pull Front",
            descricao: "Posição: Em prancha (mãos e pés no chão, corpo em linha reta). Execução: Eleve uma perna sem perder o alinhamento do corpo, depois abaixe e alterne.",
            nivel: "Nível: Intermediário",
            local: "Local: Solo",
            musculosTrabalhados: ["Músculos: Abdômen", " glúteos", " quadríceps", " músculos do ombro"],
            imagem: "leg_pull_front.jpg",
            link: "fLYsUbi_f-A",
            video: "leg_pull_front.mp4"
            },
            {
            nome: "Leg Pull Back",
            descricao: "Posição: Sentado, com as mãos no chão atrás do corpo e as pernas estendidas à frente. Execução: Eleve o corpo para uma posição de prancha invertida e levante uma perna de cada vez.",
            nivel: "Nível: Intermediário",
            local: "Local: Solo",
            musculosTrabalhados: ["Músculos: Glúteos", " isquiotibiais", " paravertebrais", " tríceps"],
            imagem: "leg_pull_back.jpg",
            link: "f_gzE5tOPQw",
            video: "leg_pull_back.mp4"
            },
            {
            nome: "Hip Circles",
            descricao: "Posição: Sentado com as pernas elevadas em um V e as mãos no chão atrás do corpo para apoio. Execução: Faça círculos com as pernas, mantendo o core ativo e o controle do movimento.",
            nivel: "Nível: Intermediário",
            local: "Local: Solo",
            musculosTrabalhados: ["Músculos: Abdômen", " flexores do quadril", " oblíquos"],
            imagem: "hip_circles.jpg",
            link: "dN1elhfKzYo",
            video: "hip_circles.mp4"
            },
            {
            nome: "Jackknife",
            descricao: "Posição: Deitado de costas com as pernas estendidas para cima. Execução: Eleve as pernas e o quadril do chão, rolando para cima e formando um ângulo com o corpo, e depois retorne lentamente.",
            nivel: "Nível: Intermediário",
            local: "Local: Solo",
            musculosTrabalhados: ["Músculos: Abdômen", " paravertebrais", " flexores do quadril", " glúteos"],
            imagem: "jackknife.jpg",
            link: "OA1MloQcPu0",
            video: "jackknife.mp4"
            },
            {
            nome: "Saw",
            descricao: "Posição: Sentado com as pernas estendidas em V e os braços abertos. Execução: Gire o tronco e estenda a mão em direção ao pé oposto, como se estivesse serrando o pé, e depois volte ao centro.",
            nivel: "Nível: Intermediário",
            local: "Local: Solo",
            musculosTrabalhados: ["Músculos: Oblíquos", " abdômen", " isquiotibiais"],
            imagem: "saw.jpg",
            link: "1XcU-WsTcaU",
            video: "saw.mp4"
            },
            {
              nome: "Up and Down",
              descricao: "Posição: Deitado de lado, corpo alinhado e uma perna sobre a outra. Execução: Realiza a dorsiflexão e movimente a perna de cima, faça a flexão plantar e retorne a perna. Inicie um novo ciclo.",
              nivel: "Nível: Básico",
              local: "Local: Solo",
              musculosTrabalhados: ["Músculos: Abdutores do quadril", " glúteos"],
              imagem: "up_and_down.jpg",
              link: "I0jwEZUA5p8",
              video: "up_and_down.mp4"
              },
              {
                nome: "Control Balance",
                descricao: "Posição: Deitado de costas com as pernas elevadas e o quadril no ar, apoiado nos ombros. Execução: Abaixe uma perna em direção ao chão enquanto a outra permanece elevada, alternando as pernas.",
                nivel: "Nível: Avançado",
                local: "Local: Solo",
                musculosTrabalhados: ["Músculos: Abdômen", " glúteos", " paravertebrais", " flexores do quadril"],
                imagem: "control_balance.jpg",
                link: "qH8b1m3GV0A",
                video: "control_balance.mp4"
                },
                {
                nome: "Boomerang",
                descricao: "Posição: Sentado com as pernas cruzadas e os braços estendidos ao lado. Execução: Role para trás, elevando as pernas acima da cabeça, depois role para frente e estenda os braços e as pernas para equilibrar-se.",
                nivel: "Nível: Avançado",
                local: "Local: Solo",
                musculosTrabalhados: ["Músculos: Abdômen", " paravertebrais", " flexores do quadril"],
                imagem: "boomerang.jpg",
                link: "SRonJI25raE",
                video: "boomerang.mp4"
                },
                {
                nome: "Corkscrew",
                descricao: "Posição: Deitado de costas com as pernas estendidas para cima. Execução: Faça pequenos círculos com as pernas juntas, movendo o quadril de lado a lado enquanto mantém o core ativado.",
                nivel: "Nível: Avançado",
                local: "Local: Solo",
                musculosTrabalhados: ["Músculos: Abdômen", " oblíquos", " paravertebrais"],
                imagem: "corkscrew.jpg",
                link: "Go6UA7SHdoE",
                video: "corkscrew.mp4"
                },
                {
                nome: "Neck Pull",
                descricao: "Posição: Deitado de costas com as pernas estendidas e as mãos atrás da cabeça. Execução: Enrole o tronco para cima, puxando o pescoço suavemente para frente, e depois retorne lentamente.",
                nivel: "Nível: Avançado",
                local: "Local: Solo",
                musculosTrabalhados: ["Músculos: Abdômen", " paravertebrais", " flexores do quadril"],
                imagem: "neck_pull.jpg",
                link: "wVaSdDSxKwY",
                video: "neck_pull.mp4"
                },
                {
                nome: "Rocking",
                descricao: "Posição: Deitado de bruços com as mãos segurando os tornozelos. Execução: Levante o peito e as pernas simultaneamente, criando um arco com o corpo, e balance para frente e para trás.",
                nivel: "Nível: Avançado",
                local: "Local: Solo",
                musculosTrabalhados: ["Músculos: Paravertebrais", " glúteos", " isquiotibiais"],
                imagem: "rocking.jpg",
                link: "KQWHWl7yV9g",
                video: "rocking.mp4"
                },
                {
                nome: "Scissors in the Air",
                descricao: "Posição: Deitado de costas com o quadril elevado e as pernas estendidas para cima. Execução: Abaixe uma perna em direção ao chão enquanto a outra permanece elevada, alternando rapidamente.",
                nivel: "Nível: Avançado",
                local: "Local: Solo",
                musculosTrabalhados: ["Músculos: Abdômen", " flexores do quadril", " isquiotibiais"],
                imagem: "scissors_in_the_air.jpg",
                link: "CO9_19vfCf4",
                video: "scissors_in_the_air.mp4"
                },
                {
                nome: "Bicycle",
                descricao: "Posição: Deitado de costas com o quadril elevado e as pernas estendidas para cima. Execução: Faça o movimento de pedalar com as pernas enquanto mantém o quadril estável.",
                nivel: "Nível: Avançado",
                local: "Local: Solo",
                musculosTrabalhados: ["Músculos: Abdômen", " quadríceps", " isquiotibiais", " glúteos"],
                imagem: "bicycle.jpg",
                link: "zKzRqV3cjgQ",
                video: "bicycle.mp4"
                },
                {
                nome: "Star",
                descricao: "Posição: Em prancha lateral, apoiado em um braço. Execução: Eleve o corpo e a perna de cima, formando uma estrela, e mantenha o equilíbrio.",
                nivel: "Nível: Avançado",
                local: "Local: Solo",
                musculosTrabalhados: ["Músculos: Abdômen", " oblíquos",  " glúteos", "ombros"],
                imagem: "star.jpg",
                link: "4J8BLfxI3TM",
                video: "star.mp4"
                },
                {
                nome: "Twist",
                descricao: "Posição: Sentado com as pernas estendidas e os braços abertos para os lados. Execução: Gire o tronco de um lado para o outro, mantendo a postura ereta.",
                nivel: "Nível: Avançado",
                local: "Local: Solo",
                musculosTrabalhados: ["Músculos: Oblíquos", " abdômen", " paravertebrais"],
                imagem: "twist.jpg",
                link: "XbQzj8rjBbw",
                video: "twist.mp4"
                },
                {
                nome: "Push-Up Series",
                descricao: "Posição: Em pé, com os pés juntos e braços ao lado do corpo. Execução: Enrole a coluna para baixo até chegar à posição de prancha. Realize flexões de braço, depois retorne à posição inicial, desenrolando a coluna.",
                nivel: "Nível: Avançado",
                local: "Local: Solo",
                musculosTrabalhados: ["Músculos: Peitorais", " deltoides", " tríceps", " abdômen", " paravertebrais"],
                imagem: "push_up_series.jpg",
                link: "Ny0qFffcemg",
                video: "push_up_series.mp4"
                },

                // SOLO = 43
                
// Exercícios no REFORMER
                // nível básico
                {
                  nome: "Footwork",
                  descricao: "Posição: Deitado de costas na plataforma do Reformer, com os pés na barra de pés e as pernas dobradas. Execução: Empurre a plataforma com os pés até estender as pernas completamente, depois retorne controladamente à posição inicial.",
                  nivel: "Nível: Básico",
                  local: "Local: Reformer",
                  musculosTrabalhados: ["Músculos: Quadríceps", " glúteos", " isquiotibiais", " panturrilhas"],
                  imagem: "footwork.jpg",
                  link: "QDBzjlhKsco",
                  video: "footwork.mp4"
                  },
                  {
                  nome: "The Hundred",
                  descricao: "Posição: Deitado de costas na plataforma do Reformer com as alças nas mãos e pernas elevadas na posição de mesa ou estendidas. Execução: Eleve a cabeça e os ombros do Reformer, bombeie os braços para cima e para baixo, inspirando por 5 contagens e expirando por 5, até completar 100.",
                  nivel: "Nível: Básico",
                  local: "Local: Reformer",
                  musculosTrabalhados: ["Músculos: Abdômen", " flexores do quadril", " músculos respiratórios"],
                  imagem: "the_hundred.jpg",
                  link: "vx4DdBWY6kQ",
                  video: "the_hundred.mp4"
                  },
                  {
                  nome: "Leg Circles",
                  descricao: "Posição: Deitado de costas na plataforma com os pés nas alças. Execução: Faça círculos amplos e controlados com as pernas, primeiro em um sentido e depois no outro, mantendo o core ativado.",
                  nivel: "Nível: Básico",
                  local: "Local: Reformer",
                  musculosTrabalhados: ["Músculos: Abdômen", " adutores e abdutores do quadril", " flexores do quadril"],
                  imagem: "leg_circles.jpg",
                  link: "C-Q__c002a0",
                  video: "leg_circles.mp4"
                  },
                  {
                  nome: "Frog",
                  descricao: "Posição: Deitado de costas na plataforma com os pés nas alças, pernas dobradas e calcanhares juntos, formando um diamante com as pernas. Execução: Estenda as pernas para frente enquanto mantém os calcanhares juntos, depois retorne à posição inicial.",
                  nivel: "Nível: Básico",
                  local: "Local: Reformer",
                  musculosTrabalhados: ["Músculos: Abdômen", " adutores", " quadríceps", " glúteos"],
                  imagem: "frog.jpg",
                  link: "ttdHtVohNY8",
                  video: "frog.mp4"
                  },
                  {
                  nome: "Knee Stretch Round Back",
                  descricao: "Posição: Em quatro apoios, com os joelhos na plataforma e as mãos segurando a barra de apoio, coluna arredondada. Execução: Empurre a plataforma para trás com os joelhos, mantendo a coluna arredondada, depois retorne controladamente.",
                  nivel: "Nível: Básico",
                  local: "Local: Reformer",
                  musculosTrabalhados: ["Músculos: Abdômen", " glúteos", " quadríceps"],
                  imagem: "knee_stretch_round_back.jpg",
                  link: "3qohz790DpQ",
                  video: "knee_stretch_round_back.mp4"
                  },
                  {
                  nome: "Knee Stretch Flat Back",
                  descricao: "Posição: Em quatro apoios, com os joelhos na plataforma e as mãos segurando a barra de apoio, com a coluna neutra (reta). Execução: Empurre a plataforma com os joelhos, mantendo as costas retas, e depois retorne.",
                  nivel: "Nível: Básico",
                  local: "Local: Reformer",
                  musculosTrabalhados: ["Músculos: Abdômen", " glúteos", " quadríceps", " músculos da coluna"],
                  imagem: "knee_stretch_flat_back.jpg",
                  link: "A3vkIu44sVs",
                  video: "knee_stretch_flat_back.mp4"
                  },
                  {
                  nome: "Elephant",
                  descricao: "Posição: Em pé na plataforma, mãos segurando a barra de apoio, coluna arredondada e pés paralelos na parte móvel. Execução: Empurre a plataforma para trás com os calcanhares, mantendo a coluna arredondada e os ombros estáveis, e depois retorne.",
                  nivel: "Nível: Básico",
                  local: "Local: Reformer",
                  musculosTrabalhados: ["Músculos: Isquiotibiais", " glúteos", " músculos da coluna", " ombros"],
                  imagem: "elephant.jpg",
                  link: "weVTzC3S9IM",
                  video: "elephant.mp4"
                  },
                  {
                  nome: "Long Stretch",
                  descricao: "Posição: Em prancha, com as mãos segurando a barra de apoio e os pés na plataforma móvel. Execução: Empurre a plataforma para trás com os pés, mantendo o corpo em linha reta, e depois retorne.",
                  nivel: "Nível: Básico",
                  local: "Local: Reformer",
                  musculosTrabalhados: ["Músculos: Abdômen", " ombros", " glúteos", " quadríceps"],
                  imagem: "long_stretch.jpg",
                  link: "BfRY99Ka-zQ",
                  video: "long_stretch.mp4"
                  },
                  {
                  nome: "Short Spine",
                  descricao: "Posição: Deitado de costas, pés nas alças e pernas estendidas. Execução: Eleve o quadril e role a coluna para cima, depois retorne, vértebra por vértebra, até a posição inicial.",
                  nivel: "Nível: Básico",
                  local: "Local: Reformer",
                  musculosTrabalhados: ["Músculos: Abdômen", " paravertebrais", " isquiotibiais", " glúteos"],
                  imagem: "short_spine.jpg",
                  link: "qcP6Ewhw7nA",
                  video: "short_spine.mp4"
                  },
                  {
                  nome: "Stomach Massage",
                  descricao: "Posição: Sentado na plataforma com os pés na barra de pés, joelhos dobrados e mãos segurando a barra de apoio ou os ombros. Execução: Empurre a plataforma para trás até estender as pernas, e depois retorne mantendo a coluna arredondada.",
                  nivel: "Nível: Básico",
                  local: "Local: Reformer",
                  musculosTrabalhados: ["Músculos: Abdômen", " quadríceps", " glúteos", " isquiotibiais"],
                  imagem: "stomach_massage.jpg",
                  link: "Wt_horWSoA4",
                  video: "stomach_massage.mp4"
                  },

                  // Nível Intermediário  
                  {
                    nome: "Coordination",
                    descricao: "Posição: Deitado de costas na plataforma com as alças nas mãos e pernas estendidas. Execução: Dobre os cotovelos e joelhos simultaneamente, e depois estenda ambos, sincronizando os movimentos.",
                    nivel: "Nível: Intermediário",
                    local: "Local: Reformer",
                    musculosTrabalhados: ["Músculos: Abdômen", " flexores do quadril", " bíceps"],
                    imagem: "coordination.jpg",
                    link: "3dYwRDMassE",
                    video: "coordination.mp4"
                    },
                    {
                    nome: "Swan",
                    descricao: "Posição: Deitado de bruços na plataforma, segurando a barra de apoio. Execução: Eleve o peito e a cabeça enquanto estende os braços, criando uma curva suave na coluna.",
                    nivel: "Nível: Intermediário",
                    local: "Local: Reformer",
                    musculosTrabalhados: ["Músculos: Paravertebrais", " glúteos", " ombros"],
                    imagem: "swan.jpg",
                    link: "YaoTIbiHb0w",
                    video: "swan.mp4"
                    },
                    {
                    nome: "Long Box Pulling Straps",
                    descricao: "Posição: Deitado de bruços sobre a caixa longa, segurando as alças. Execução: Puxe as alças para trás, mantendo os braços estendidos e elevando o peito do Reformer.",
                    nivel: "Nível: Intermediário",
                    local: "Local: Reformer",
                    musculosTrabalhados: ["Músculos: Paravertebrais", " glúteos", " deltoides"],
                    imagem: "long_box_pulling_straps.jpg",
                    link: "Zic_h-K3gSI",
                    video: "long_box_pulling_straps.mp4"
                    },
                    {
                    nome: "Backstroke",
                    descricao: "Posição: Sentado na caixa longa, de costas para a barra de apoio, com as alças nas mãos. Execução: Eleve os braços e pernas simultaneamente em uma ação coordenada de natação, mantendo o core ativado.",
                    nivel: "Nível: Intermediário",
                    local: "Local: Reformer",
                    musculosTrabalhados: ["Músculos: Abdômen", " ombros", " quadríceps"],
                    imagem: "backstroke.jpg",
                    link: "aT8BFRmEsBQ",
                    video: "backstroke.mp4"
                    },
                    {
                    nome: "Teaser",
                    descricao: "Posição: Deitado de costas na plataforma, segurando as alças, com as pernas elevadas. Execução: Eleve o tronco e as pernas simultaneamente até formar um V, e depois retorne com controle.",
                    nivel: "Nível: Intermediário",
                    local: "Local: Reformer",
                    musculosTrabalhados: ["Músculos: Abdômen", " flexores do quadril", " paravertebrais"],
                    imagem: "teaser.jpg",
                    link: "6j_r7O7FgRs",
                    video: "teaser.mp4"
                    },
                    {
                    nome: "Mermaid",
                    descricao: "Posição: Sentado lateralmente na plataforma com as pernas cruzadas e uma mão segurando a barra de apoio. Execução: Alongue o tronco lateralmente enquanto empurra a plataforma com a mão oposta, depois retorne à posição inicial.",
                    nivel: "Nível: Intermediário",
                    local: "Local: Reformer",
                    musculosTrabalhados: ["Músculos: Oblíquos", " paravertebrais", " ombros"],
                    imagem: "mermaid.jpg",
                    link: "Fi5S7cHGkMI",
                    video: "mermaid.mp4"
                    },
                    {
                    nome: "Long Spine",
                    descricao: "Posição: Deitado de costas na plataforma, com os pés nas alças. Execução: Eleve as pernas e o quadril, rolando a coluna para cima e para baixo, controladamente.",
                    nivel: "Nível: Intermediário",
                    local: "Local: Reformer",
                    musculosTrabalhados: ["Músculos: Abdômen", " glúteos", " paravertebrais", " isquiotibiais"],
                    imagem: "long_spine.jpg",
                    link: "tiFjck9xVus",
                    video: "long_spine.mp4"
                    },
                                    
                      // Nível Avançado
                      {
                        nome: "Tendon Stretch",
                        descricao: "Posição: Sentado de frente para a barra de pés, com as mãos segurando-a e os pés na plataforma móvel. Execução: Empurre a plataforma para trás, mantendo o tronco arredondado e depois retorne à posição inicial.",
                        nivel: "Nível: Avançado",
                        local: "Local: Reformer",
                        musculosTrabalhados: ["Músculos: Abdômen", " isquiotibiais", " glúteos"],
                        imagem: "tendon_stretch.jpg",
                        link: "7TDdzbuLiUI",
                        video: "tendon_stretch.mp4"
                        },
                        {
                        nome: "Snake",
                        descricao: "Posição: Em prancha lateral com as mãos segurando a barra de apoio e os pés na plataforma móvel. Execução: Empurre a plataforma enquanto gira o tronco e eleva o quadril, depois retorne.",
                        nivel: "Nível: Avançado",
                        local: "Local: Reformer",
                        musculosTrabalhados: ["Músculos: Abdômen", " oblíquos", " paravertebrais", " ombros"],
                        imagem: "snake.jpg",
                        link: "L2zuXWE-ZKY",
                        video: "snake.mp4"
                        },
                        {
                        nome: "Snake Twist",
                        descricao: "Posição: Em prancha lateral, segurando a barra de apoio com uma mão, pés na plataforma móvel. Execução: Gire o tronco enquanto empurra a plataforma, formando uma torção na coluna, e retorne.",
                        nivel: "Nível: Avançado",
                        local: "Local: Reformer",
                        musculosTrabalhados: ["Músculos: Oblíquos", " abdômen", " glúteos"],
                        imagem: "snake_twist.jpg",
                        link: "kvZb4YSSEfY",
                        video: "snake_twist.mp4"
                        },
                        {
                        nome: "Balance Control Front",
                        descricao: "Posição: Em prancha frontal com as mãos segurando a barra de apoio e os pés na plataforma móvel. Execução: Empurre a plataforma para trás, mantendo o corpo em linha reta, e depois retorne à posição inicial.",
                        nivel: "Nível: Avançado",
                        local: "Local: Reformer",
                        musculosTrabalhados: ["Músculos: Abdômen", " ombros", " glúteos", " quadríceps"],
                        imagem: "balance_control_front.jpg",
                        link: "IxU3qHvxxlw",
                        video: "balance_control_front.mp4"
                        },
                        {
                        nome: "Balance Control Back",
                        descricao: "Posição: Sentado na plataforma do Reformer, com as mãos apoiadas na barra e as pernas estendidas. Execução: Eleve o corpo e as pernas em uma posição de prancha invertida, mantendo o equilíbrio e o controle.",
                        nivel: "Nível: Avançado",
                        local: "Local: Reformer",
                        musculosTrabalhados: ["Músculos: Glúteos", " isquiotibiais", " paravertebrais", " tríceps"],
                        imagem: "balance_control_back.jpg",
                        link: "P5yqvfHMNpo",
                        video: "balance_control_back.mp4"
                        },
                        {
                        nome: "Star",
                        descricao: "Posição: Em prancha lateral, com uma mão segurando a barra de apoio e os pés na plataforma móvel. Execução: Eleve o corpo lateralmente enquanto levanta a perna de cima, formando uma posição de estrela, e depois retorne.",
                        nivel: "Nível: Avançado",
                        local: "Local: Reformer",
                        musculosTrabalhados: ["Músculos: Abdômen", " glúteos", " oblíquos", " ombros"],
                        imagem: "star.jpg",
                        link: "Edow6Q-Mw5Q",
                        video: "star.mp4"
                        },
                        {
                        nome: "Russian Splits",
                        descricao: "Posição: Um pé na barra de apoio e o outro na plataforma móvel, em posição de estocada. Execução: Empurre a plataforma para trás, estendendo a perna traseira, e depois retorne, mantendo o equilíbrio e o controle do tronco.",
                        nivel: "Nível: Avançado",
                        local: "Local: Reformer",
                        musculosTrabalhados: ["Músculos: Quadríceps", " glúteos", " isquiotibiais", " abdômen"],
                        imagem: "russian_splits.jpg",
                        link: "Po9NpVojl74",
                        video: "russian_splits.mp4"
                        },
                        {
                        nome: "Arabesque",
                        descricao: "Posição: Em pé sobre a plataforma, de frente para a barra de apoio, com uma perna estendida para trás e os braços segurando a barra. Execução: Empurre a plataforma para trás enquanto mantém a perna elevada e o tronco estável. Retorne à posição inicial com controle.",
                        nivel: "Nível: Avançado",
                        local: "Local: Reformer",
                        musculosTrabalhados: ["Músculos: Abdômen", " glúteos", " paravertebrais", " isquiotibiais"],
                        imagem: "arabesque.jpg",
                        link: "0Is09CTjxKU",
                        video: "arabesque.mp4"
                        },
                        {
                        nome: "Reverse Knee Stretch",
                        descricao: "Posição: De joelhos na plataforma, com as mãos segurando a barra de apoio. Execução: Mantenha os braços fixos e puxe a plataforma para frente com os joelhos, mantendo o core ativado e a coluna estável.",
                        nivel: "Nível: Avançado",
                        local: "Local: Reformer",
                        musculosTrabalhados: ["Músculos: Abdômen", " glúteos", " quadríceps"],
                        imagem: "reverse_knee_stretch.jpg",
                        link: "naxsHoeWCDY",
                        video: "reverse_knee_stretch.mp4"
                        },
                      //  Reformer = 26 



// // Exercícios no CHAIR
//                 // nível básico
                {
                  nome: "Abdominal Work Upper",
                  descricao: "Posição: Sentado no chão, com as mãos segurando o pedal e os pés posicionados em cima da cadeira. Execução: Pressione o pedal para baixo com as mãos, mantendo a posição.",
                  nivel: "Nível: Avançado",
                  local: "Local: Chair",
                  musculosTrabalhados: ["Músculos: Alongamento de Isquiotibiais", " abdômen"],
                  imagem: "Abdominal Work Upper.jpg",
                  link: "i4vssPUZuPc",
                  video: "Abdominal Work Upper.mp4"
              },
              {
                  nome: "Frog on the Chair",
                  descricao: "Posição: Deitado no chão com as pernas dobradas e os pés posicionados no pedal. Execução: Pressione o pedal para baixo estendendo as pernas, mantendo os joelhos abertos e o core ativado.",
                  nivel: "Nível: Básico",
                  local: "Local: Chair",
                  musculosTrabalhados: ["Músculos: Abdômen", " adutores", " quadríceps", " glúteos"],
                  imagem: "frog_on_the_chair.jpg",
                  link: "sxQfLHkpd0U",
                  video: "frog_on_the_chair.mp4"
              },
              {
                  nome: "Wrap Toes & Heels",
                  descricao: "Posição: Sentado na chair com os pés apoiados no pedal. Execução: Execute o movimento dos MMII.",
                  nivel: "Nível: Básico",
                  local: "Local: Chair",
                  musculosTrabalhados: ["Músculos: Quadríceps", " Tronco", " Perna"],
                  imagem: "Wrap Toes & Heels.jpg",
                  link: "MJrPI3SmcY8",
                  video: "Wrap Toes & Heels.mp4"
              },
              {
                  nome: "Press Pedal with Arms",
                  descricao: "Posição: Sentado em frente a chair, com as mãos segurando o pedal. Execução: Pressione o pedal para baixo com as mães, mantendo o tronco estável e o core ativado.",
                  nivel: "Nível: Básico",
                  local: "Local: Chair",
                  musculosTrabalhados: ["Músculos: Ombros", " tríceps", " abdômen"],
                  imagem: "Press Pedal with Arms.jpg",
                  link: "zvH8KbZwWeI",
                  video: "Press Pedal with Arms.mp4"
              },
              {
                  nome: "Abdominal Work Lower",
                  descricao: "Posição: Sentado em frente a chair, com as mãos segurando o pedal e os pés posicionados em cima da cadeira. Execução: Pressione o pedal para baixo com as mãos, realizado flexão de tronco com deslocamento para trás.",
                  nivel: "Nível: Avançado",
                  local: "Local: Chair",
                  musculosTrabalhados: ["Músculos: Alongamento de Isquiotibiais", " abdômen"],
                  imagem: "Abdominal Work Lower.jpg",
                  link: "rokE3_o0JXA",
                  video: "Abdominal Work Lower.mp4"
              },
              {
                nome: "Footwork",
                descricao: "Posição: Sentado na chair com a coluna ereta e os pés posicionados no pedal. Execução: Pressione a barra para baixo com os pés, estendendo as pernas, e depois retorne à posição inicial controladamente.",
                nivel: "Nível: Básico",
                local: "Local: Chair",
                musculosTrabalhados: ["Músculos: Quadríceps", " glúteos", " isquiotibiais", " panturrilhas"],
                imagem: "footwork_chair.jpg",
                link: "mzOunMJK9gk",
                video: "footwork_chair.mp4"
            },
            {
                nome: "Standing Pump",
                descricao: "Posição: Em pé na frente da chair, com um pé no pedal e o outro no chão. Execução: Pressione o pedal para baixo com o pé enquanto mantém o corpo ereto e o core ativado, depois retorne controladamente.",
                nivel: "Nível: Básico",
                local: "Local: Chair",
                musculosTrabalhados: ["Músculos: Quadríceps", " glúteos", " abdômen"],
                imagem: "standing_pump_chair.jpg",
                link: "IsOW-vMt2sw",
                video: "standing_pump_chair.mp4"
            },
            {
                nome: "Jackknife",
                descricao: "Posição: Deitado no chão com as mãos apoiadas no pedal e as pernas extendidas no chão. Execução: Realize o movimento levando as pernas em direção a chair, segurando no pedal.",
                nivel: "Nível: Avançado",
                local: "Local: Chair",
                musculosTrabalhados: ["Músculos: Quadríceps", " abdômen", " Alongamento de isquiotibiais"],
                imagem: "Jackknife.jpg",
                link: "l8jAaKnTyX0",
                video: "Jackknife.mp4"
            },
            {
                nome: "One Arm Press",
                descricao: "Sentado de costas para a barra de pedal, com uma mão segurando a barra atrás do corpo. Execução: Pressione a barra para baixo e depois retorne à posição inicial.",
                nivel: "Nível: Básico",
                local: "Local: Chair",
                musculosTrabalhados: ["Músculos: Tríceps", " deltoides", " abdômen"],
                imagem: "One Arm Press.jpg",
                link: "yRJn_WDxzMY",
                video: "One Arm Press.mp4"
            },
            {
                nome: "Triceps Press Sit",
                descricao: "Posição: Sentado de costas para a barra de pedal, com as mãos segurando a barra atrás do corpo. Execução: Pressione a barra para baixo estendendo os braços e depois retorne à posição inicial.",
                nivel: "Nível: Básico",
                local: "Local: Chair",
                musculosTrabalhados: ["Músculos: Tríceps", " deltoides", " abdômen"],
                imagem: "triceps_press_sit_chair.jpg",
                link: "CnWO_t74Ykk",
                video: "triceps_press_sit_chair.mp4"
            },
            {
              nome: "Twist Lean",
              descricao: "Posição: Sentado na parte de trás da chair com as costas para o pedal. Execução: Realize o movimento rodando o tronco a abaixando até encontrar o pedal com uma das mãos. Abaixe realizandpo a extensão de tronco, abaixando o pedal e elevando a perna no mesmo lado.",
              nivel: "Nível: Básico",
              local: "Local: Chair",
              musculosTrabalhados: ["Músculos: Panturrilhas", " quadríceps", " glúteos"],
              imagem: "Twist Lean.jpg",
              link: "s0WOB666BDM",
              video: "Twist Lean.mp4"
          },
          {
              nome: "Sitting Balanced Toe Taps",
              descricao: "Posição: Sentado no assento da Chair segurando no apoio. Execução: Realize o movimento alternando as pernas, mantendo o tronco estável e o abdômen contraído.",
              nivel: "Nível: Básico",
              local: "Local: Chair",
              musculosTrabalhados: ["Músculos: Flexores de Quadril", " abdômen"],
              imagem: "Sitting Balanced Toe Taps.jpg",
              link: "Eb0TWWcet8E",
              video: "Sitting Balanced Toe Taps.mp4"
          },
          {
            nome: "Leg Press Down",
            descricao: "Posição: Sentado na chair com a coluna ereta e os pés posicionados no pedal. Execução: Pressione o pedal para baixo mantendo o tronco alinhado.",
            nivel: "Nível: Básico",
            local: "Local: Chair",
            musculosTrabalhados: ["Músculos: Quadríceps", " Tronco"],
            imagem: "Leg Press Down.jpg",
            link: "j_ko5hxEPOU",
            video: "Leg Press Down.mp4"
          },
          {
            nome: "Leg Press Down With Arm",
            descricao: "Posição: Sentado na chair com a coluna ereta e os pés posicionados no pedal e as mãos para cima. Execução: Pressione o pedal para baixo com os braços para cima e retorne o pedal abaixando os braços.",
            nivel: "Nível: Básico",
            local: "Local: Chair",
            musculosTrabalhados: ["Músculos: Quadríceps", " Tronco"],
            imagem: "Leg Press Down With Arm.jpg",
            link: "ICIu5PrvMt4",
            video: "Leg Press Down With Arm.mp4"
          },
          {
            nome: "Prone Scapular",
            descricao: "Posição: Deitado em ventral na chair com a coluna ereta, os pés posicionados para fora e as mãos no pedal. Execução: Com os cotolevos estendidos, realize o movimento de protração empurrando o pedal para baixo e retorne.",
            nivel: "Nível: Intermediário",
            local: "Local: Chair",
            musculosTrabalhados: ["Músculos: Tronco", " Serrátil"],
            imagem: "Prone Scapular.jpg",
            link: "62xbhYRhyT0",
            video: "Prone Scapular.mp4"
          },



          {
            nome: "Double Leg Stretch",
            descricao: "Posição: Sentado no assento da Chair segurando no apoio, com as pernas fletidas. Execução: Realize o movimento estendendo os joelhos, mantendo o tronco estável e o abdômen contraído.",
            nivel: "Nível: Intermediário",
            local: "Local: Chair",
            musculosTrabalhados: ["Músculos: Flexores de Quadril", " Alongamento de Isquitibiais", " abdômen"],
            imagem: "Double Leg Stretch.jpg",
            link: "vslekaxbxI4",
            video: "Double Leg Stretch.mp4"
          },
          {
              nome: "Side Pull Up",
              descricao: "Posição: Em pé lateralmente na barra de pedal, com uma mão segurando a lateral da chair e o outro braço ao longo do corpo. Execução: Eleve o quadril para o ar, puxando o pedal com os pés e ativando os músculos laterais do tronco.",
              nivel: "Nível: Intermediário",
              local: "Local: Chair",
              musculosTrabalhados: ["Músculos: Oblíquos", " abdômen", " paravertebrais", " glúteos"],
              imagem: "side_pull_up_chair.jpg",
              link: "RulzmsLXzss",
              video: "side_pull_up_chair.mp4"
          },
          {
            nome: "Standing Scapula Movement",
            descricao: "Posição: Em pé de costas em cima da chair com as mãos segurando no apoio de mãos com os cotovelos estendidos. Execução: Com os cotovelos extendidos, realize o movimento nos ombros e mantenha o tronco estável.",
            nivel: "Nível: Intermediário",
            local: "Local: Chair",
            musculosTrabalhados: ["Músculos: Tríceps", " Trapézio Inferior", " Tronco"],
            imagem: "Standing Scapula Movement.jpg",
            link: "Rf4KhiAsFIQ",
            video: "Standing Scapula Movement.mp4"
          },
          {
            nome: "Standing Tríceps Movement",
            descricao: "Posição: Em pé de costas em cima da chair com as mãos segurando no apoio de mãos com os cotovelos estendidos. Execução: Com os cotovelos extendidos, realize o movimento fletindo e estendendo os cotovelos.",
            nivel: "Nível: Intermediário",
            local: "Local: Chair",
            musculosTrabalhados: ["Músculos: Tríceps", " Trapézio Inferior", " Tronco"],
            imagem: "Standing Tríceps Movement.jpg",
            link: "PFWDhlXwOxU",
            video: "Standing Tríceps Movement.mp4"
          },
//             // nível Avançado
            {
              nome: "Sitting Pull Up",
              descricao: "Posição: Sentado no pedal da chair, com as pernas estendidas e as mãos segurando no apoio. Execução: Mantenha a perna com os joelhos estendidos, o tronco estável e realize o movimento no cotovelo.",
              nivel: "Nível: Avançado",
              local: "Local: Chair",
              musculosTrabalhados: ["Músculos: Quadríceps", " Iliopsoas", " abdômen", " Tríceps"],
              imagem: "Sitting Pull Up.jpg",
              link: "1T18WQ1nhY8",
              video: "Sitting Pull Up.mp4"
          },
          {
              nome: "Teaser on the Chair",
              descricao: "Posição: Sentado no assento da chair, com as pernas estendidas e os braços segurando o pedal. Execução: Eleve o tronco e as pernas simultaneamente, formando um “V”, enquanto pressiona o pedal.",
              nivel: "Nível: Avançado",
              local: "Local: Chair",
              musculosTrabalhados: ["Músculos: Abdômen", " paravertebrais", " quadríceps", " glúteos"],
              imagem: "teaser_on_chair.jpg",
              link: "33XdlARe54Y",
              video: "teaser_on_chair.mp4"
          },
          {
              nome: "Roll Back",
              descricao: "Posição: Sendato na chair de costas para o pedal. Execução: Faça um rolamento para trás até encostar as mãos no pedal e pressione abaixando.",
              nivel: "Nível: Avançado",
              local: "Local: Chair",
              musculosTrabalhados: ["Músculos: Abdutores", " glúteos", " abdômen"],
              imagem: "Roll Back.jpg",
              link: "Hou2gcmCojQ",
              video: "Roll Back.mp4"
          },
              //  Chair = 22



// Exercícios no CADILLAC
          // nível básico
          {
            nome: "Roll Down",
            descricao: "Posição: Sentado de frente para a barra de roll down, segurando a barra com os braços estendidos. Execução: Enrole a coluna para trás, abaixando lentamente até deitar no Cadillac, depois retorne à posição inicial.",
            nivel: "Nível: Básico",
            local: "Local: Cadillac",
            musculosTrabalhados: ["Músculos: Abdômen", " paravertebrais", " ombros"],
            imagem: "roll_down.jpg",
            link: "aTE_nrAyC5s",
            video: "roll_down.mp4"
          },
          {
            nome: "Leg Circles",
            descricao: "Posição: Deitado de costas, com as alças presas aos pés. Execução: Faça círculos controlados com as pernas, mantendo o core estável.",
            nivel: "Nível: Básico",
            local: "Local: Cadillac",
            musculosTrabalhados: ["Músculos: Quadríceps", " glúteos", " abdômen"],
            imagem: "leg_circles.jpg",
            link: "JcgVWZZ0YPc",
            video: "leg_circles.mp4"
          },
          {
            nome: "Sitting Cat Stretch",
            descricao: "Posição: Sentado de frente para a Barra. Execução: Empurre a barra realizando uma flexão de coluna e ao final faça a correção.",
            nivel: "Nível: Básico",
            local: "Local: Cadillac",
            musculosTrabalhados: ["Músculos: Alongamento de Paravertebrais", " Mobilidade de coluna"],
            imagem: "Sitting Cat Stretch.jpg",
            link: "L2HniKCEQbQ",
            video: "Sitting Cat Stretch.mp4"
          },
          {
            nome: "Sitting Cat Stretch & Rotation",
            descricao: "Posição: Sentado de frente para a Barra. Execução: Empurre a barra com uma mão e faça uma rotação de tronco, passando a mão oposta rente ao assento.",
            nivel: "Nível: Básico",
            local: "Local: Cadillac",
            musculosTrabalhados: ["Músculos: Alongamento de Paravertebrais", " Mobilidade de coluna"],
            imagem: "Sitting Cat Stretch & Rotation.jpg",
            link: "Hsp1ZpGEOhM",
            video: "Sitting Cat Stretch & Rotation.mp4"
          },
          {
            nome: "Leg Press",
            descricao: "Posição: Deitado de costas com as pernas estendidas e os pés na barra. Execução: Empurre a barra para cima, estendendo as pernas, e depois retorne controladamente.",
            nivel: "Nível: Básico",
            local: "Local: Cadillac",
            musculosTrabalhados: ["Músculos: Quadríceps", " glúteos", " isquiotibiais"],
            imagem: "leg_press.jpg",
            link: "7qTRHjqa08c",
            video: "leg_press.mp4"
          },
          {
            nome: "Cat Stretch",
            descricao: "Posição: De joelhos em quatro apoios com as mãos segurando a barra. Execução: Enrole a coluna para dentro (flexão) e depois para fora (extensão), mobilizando a coluna.",
            nivel: "Nível: Básico",
            local: "Local: Cadillac",
            musculosTrabalhados: ["Músculos: Abdômen", " paravertebrais"],
            imagem: "cat_stretch.jpg",
            link: "GjjFGpnSCQ4",
            video: "cat_stretch.mp4"
          },
          {
            nome: "Kneeling Cat",
            descricao: "Posição: De joelhos em cima do cadillac, com as mãos segurando a barra. Execução: Enrole a coluna (flexão) e ao final faça a correção (extensão), mobilizando a coluna.",
            nivel: "Nível: Básico",
            local: "Local: Cadillac",
            musculosTrabalhados: ["Músculos: Abdômen", " paravertebrais"],
            imagem: "Kneeling Cat.jpg",
            link: "CWPS129bfz0",
            video: "Kneeling Cat.mp4"
          },
          {
            nome: "Pull-Down",
            descricao: "Posição: Sentado de costas para a barra de molas, com as mãos segurando a barra. Execução: Puxe a barra para baixo, fletindo os cotovelos.",
            nivel: "Nível: Básico",
            local: "Local: Cadillac",
            musculosTrabalhados: ["Músculos: Deltóide", " tríceps", " Tronco"],
            imagem: "Pull-Down.jpg",
            link: "WLW__DaGbUc",
            video: "Pull-Down.mp4"
          },
          {
            nome: "Side Reach",
            descricao: "Posição: De joelhos em cima do cadillac, com as mãos segurando a barra. Execução: Faça um deslocamento para trás, sem fletir o quadril e ao mesmo tempo rode o tronco realizando um alcance para trás com a uma das mãos.",
            nivel: "Nível: Básico",
            local: "Local: Cadillac",
            musculosTrabalhados: ["Músculos: Quadríceps", " tronco"],
            imagem: "Side Reach.jpg",
            link: "qAVVWGQhA4Y",
            video: "Side Reach.mp4"
          },

          // nível Intermediário
          
          {
            nome: "Teaser",
            descricao: "Posição: Deitado de costas com as as mãos na barra. Execução: Eleve o tronco e as pernas simultaneamente em uma posição de teaser.",
            nivel: "Nível: Intermediário",
            local: "Local: Cadillac",
            musculosTrabalhados: ["Músculos: Abdômen", " quadríceps"],
            imagem: "teaser.jpg",
            link: "AZvBiQfrByg",
            video: "teaser.mp4"
          },
          {
            nome: "Airplane",
            descricao: "Posição: Deitado de costas com as pernas nas alças. Execução: Eleve o quadril e as pernas para o ar, depois retorne controladamente.",
            nivel: "Nível: Intermediário",
            local: "Local: Cadillac",
            musculosTrabalhados: ["Músculos: Abdômen", " paravertebrais", " glúteos"],
            imagem: "airplane.jpg",
            link: "SVc3ROEnhDw",
            video: "airplane.mp4"
          },
          {
            nome: "Long Spine",
            descricao: "Posição: Deitado de costas com as pernas nas alças. Execução: Eleve as pernas e a coluna em uma linha reta, depois retorne devagar.",
            nivel: "Nível: Intermediário",
            local: "Local: Cadillac",
            musculosTrabalhados: ["Músculos: Abdômen", " paravertebrais", " isquiotibiais"],
            imagem: "long_spine.jpg",
            link: "ewJKm6RNCZ4",
            video: "long_spine.mp4"
          },
          {
            nome: "Mermaid",
            descricao: "Posição: Sentado de lado com uma mão segurando a barra de molas. Execução: Puxe a barra para baixo enquanto inclina o tronco lateralmente.",
            nivel: "Nível: Intermediário",
            local: "Local: Cadillac",
            musculosTrabalhados: ["Músculos: Oblíquos", " paravertebrais"],
            imagem: "mermaid.jpg",
            link: "wbgRLhqa5Y4",
            video: "mermaid.mp4"
          },
          {
            nome: "Monkey",
            descricao: "Posição: Sentado de frente para a barra de molas, segurando a barra com as mãos. Execução: Puxe a barra para baixo e eleve as pernas estendidas, depois retorne.",
            nivel: "Nível: Intermediário",
            local: "Local: Cadillac",
            musculosTrabalhados: ["Músculos: Abdômen", " quadríceps", " glúteos"],
            imagem: "monkey.jpg",
            link: "MWCQf7HsLyQ",
            video: "monkey.mp4"
          },
          {
            nome: "Leg Springs",
            descricao: "Posição: Deitado de costas com as alças nos pés. Execução: Pressione as alças para baixo e retorne controladamente.",
            nivel: "Nível: Intermediário",
            local: "Local: Cadillac",
            musculosTrabalhados: ["Músculos: Quadríceps", " glúteos", " abdômen"],
            imagem: "leg_springs.jpg",
            link: "KRg6O7uFq4w",
            video: "leg_springs.mp4"
          },
          {
            nome: "Push-Through",
            descricao: "Posição: Sentado de frente para a barra segurando com as mãos. Execução: Empurre a barra para frente enquanto enrole a coluna.",
            nivel: "Nível: Intermediário",
            local: "Local: Cadillac",
            musculosTrabalhados: ["Músculos: Abdômen", " ombros"],
            imagem: "push_through.jpg",
            link: "4x2Pz-PA2VI",
            video: "push_through.mp4"
          },
          {
            nome: "Hanging Back",
            descricao: "Posição: Deitado de costas com os pés nas alças e as mãos segurando as alças. Execução: Estenda o corpo, criando um arco suave.",
            nivel: "Nível: Intermediário",
            local: "Local: Cadillac",
            musculosTrabalhados: ["Músculos: Paravertebrais", " glúteos", " isquiotibiais"],
            imagem: "hanging_back.jpg",
            link: "syn3kB3lkeM",
            video: "hanging_back.mp4"
          },
          {
            nome: "Leg Circles with Springs",
            descricao: "Posição: Deitado de costas com as alças presas aos pés. Execução: Faça círculos amplos e controlados com as pernas, ativando o core.",
            nivel: "Nível: Intermediário",
            local: "Local: Cadillac",
            musculosTrabalhados: ["Músculos: Quadríceps", " abdômen", " glúteos"],
            imagem: "leg_circles_with_springs.jpg",
            link: "zY9g6QJGXD4",
            video: "leg_circles_with_springs.mp4"
          },
          {
            nome: "Hanging Pull Ups",
            descricao: "Posição: MMII Suspensos no trapézio com as mãos segurando as barras. Execução: Puxe o corpo para cima enquanto mantém o core ativado.",
            nivel: "Nível: Intermediário",
            local: "Local: Cadillac",
            musculosTrabalhados: ["Músculos: Bíceps", " deltoides", " abdômen"],
            imagem: "hanging_pull_ups.jpg",
            link: "bS5y7L-msxw",
            video: "hanging_pull_ups.mp4"
          },
          {
            nome: "Single Leg Stretch with Springs",
            descricao: "Posição: Deitado de costas com uma perna nas alças. Execução: Alterne as pernas em um movimento de estiramento com controle.",
            nivel: "Nível: Intermediário",
            local: "Local: Cadillac",
            musculosTrabalhados: ["Músculos: Abdômen", " quadríceps", " glúteos"],
            imagem: "single_leg_stretch_with_springs.jpg",
            link: "HJZa-Eiqn8c",
            video: "single_leg_stretch_with_springs.mp4"
          },
          {
            nome: "Kneeling Arm Circles",
            descricao: "Posição: De joelhos com as mãos segurando as alças. Execução: Faça círculos com os braços, mantendo o corpo estável.",
            nivel: "Nível: Intermediário",
            local: "Local: Cadillac",
            musculosTrabalhados: ["Músculos: Ombros", " abdômen", " paravertebrais"],
            imagem: "kneeling_arm_circles.jpg",
            link: "AOvATGjwH-w",
            video: "kneeling_arm_circles.mp4"
          },
          {
            nome: "Side-lying Leg Lifts",
            descricao: "Posição: Deitado de lado com a perna superior na alça. Execução: Levante a perna superior, mantendo o tronco estável.",
            nivel: "Nível: Intermediário",
            local: "Local: Cadillac",
            musculosTrabalhados: ["Músculos: Abdutores", " glúteos", " quadríceps"],
            imagem: "side_lying_leg_lifts.jpg",
            link: "p8wY0pJJswc",
            video: "side_lying_leg_lifts.mp4"
          },
          {
            nome: "Swan",
            descricao: "Posição: Deitado em decúbito dorsal com as mãos na barra. Execução: Faça a extensão de tronco com os cotolevos estendidos puxando a barra, sem passar da altura da cabeça.",
            nivel: "Nível: Intermediário",
            local: "Local: Cadillac",
            musculosTrabalhados: ["Músculos: Eretores", " Tríceps", " Ombro"],
            imagem: "swan.jpg",
            link: "TLKTWXJfsEg",
            video: "swan.mp4"
          },
          {
            nome: "Overhead Swan",
            descricao: "Posição: Deitado em decúbito dorsal com as mãos na barra. Execução: Faça a extensão de tronco passando a barra acima da altura da cabeça.",
            nivel: "Nível: Intermediário",
            local: "Local: Cadillac",
            musculosTrabalhados: ["Músculos: Eretores", " Tríceps", " Ombro"],
            imagem: "overhead swan.jpg",
            link: "hpZpk_JMsBw",
            video: "overhead swan.mp4"
          },
          
          // nível Avançado
          {
            nome: "Shoulder Bridge with Springs",
            descricao: "Posição: Deitado de costas com os pés apoiados e as mãos segurando as alças. Execução: Levante o quadril e pressione as alças para baixo, formando uma ponte.",
            nivel: "Nível: Avançado",
            local: "Local: Cadillac",
            musculosTrabalhados: ["Músculos: Glúteos", " isquiotibiais", " abdômen"],
            imagem: "high_bridge_with_springs.jpg",
            link: "nCxXza9VY6o",
            video: "high_bridge_with_springs.mp4"
          },
          {
            nome: "Scissors",
            descricao: "Posição: Deitado de costas com as pernas nas alças. Execução: Alterne as pernas, mantendo o core estável enquanto uma perna desce.",
            nivel: "Nível: Avançado",
            local: "Local: Cadillac",
            musculosTrabalhados: ["Músculos: Abdômen", " quadríceps", " glúteos"],
            imagem: "scissors.jpg",
            link: "q7dRreXAoeo",
            video: "scissors.mp4"
          },
          {
            nome: "Side Leg Series",
            descricao: "Posição: Deitado de lado com a perna superior na alça. Execução: Faça movimentos de elevação, círculo e extensão da perna.",
            nivel: "Nível: Avançado",
            local: "Local: Cadillac",
            musculosTrabalhados: ["Músculos: Abdutores", " glúteos", " abdômen"],
            imagem: "side_leg_series.jpg",
            link: "p8wY0pJJswc&t=35s",
            video: "side_leg_series.mp4"
          },
          {
            nome: "Teaser with Springs",
            descricao: "Posição: Deitado de costas com as alças nos pés. Execução: Eleve o tronco e as pernas simultaneamente em uma posição de teaser, controlando a descida.",
            nivel: "Nível: Avançado",
            local: "Local: Cadillac",
            musculosTrabalhados: ["Músculos: Abdômen", " quadríceps"],
            imagem: "teaser_with_springs.jpg",
            link: "FZy8COOtTd4",
            video: "teaser_with_springs.mp4"
          },
          {
            nome: "Spine Stretch",
            descricao: "Posição: Sentado com as pernas estendidas e segurando a barra. Execução: Alongue a coluna para frente enquanto pressiona empurra a barra.",
            nivel: "Nível: Avançado",
            local: "Local: Cadillac",
            musculosTrabalhados: ["Músculos: Abdômen", " paravertebrais"],
            imagem: "spine_stretch.jpg",
            link: "CS8vbqbAOtA",
            video: "spine_stretch.mp4"
          },
          {
            nome: "Backbend",
            descricao: "Posição: Ajoelhado segurando a barra de molas. Execução: Desloque o tronco para trás realizando em um arco puxando a barra para o peito juntamente com uma extensão de coluna.",
            nivel: "Nível: Avançado",
            local: "Local: Cadillac",
            musculosTrabalhados: ["Músculos: Paravertebrais", " glúteos", " abdômen"],
            imagem: "backbend.jpg",
            link: "ENL97dPa2cY",
            video: "backbend.mp4"
          },
          // Cadillac = 32





// Exercícios no BARREL

{
  nome: "Round Back",
  descricao: "Posição: Sentado no Barrel com os pés fixados na escada. Execução: Desloque o tronco para trás realizando uma flexão (rolamento) da coluna.",
  nivel: "Nível: Intermediário",
  local: "Local: Barrel",
  musculosTrabalhados: ["Músculos: abdômen"],
  imagem: "Round Back.jpg",
  link: "NrKBQHjz1mA",
  video: "Round Back.mp4"
},
{
  nome: "Twist",
  descricao: "Posição: Sentado no Barrel com os pés fixados na escada. Execução: Desloque o tronco para trás realizando uma rotação abrindo o braço para trás.",
  nivel: "Nível: Intermediário",
  local: "Local: Barrel",
  musculosTrabalhados: ["Músculos: abdômen"],
  imagem: "Twist.jpg",
  link: "tec-ZHEK0SM",
  video: "Twist.mp4"
},
{
  nome: "Flat Back",
  descricao: "Posição: Sentado no Barrel com os pés fixados na escada. Execução: Desloque o tronco para trás mantendo a coluna alinhada.",
  nivel: "Nível: Avançado",
  local: "Local: Barrel",
  musculosTrabalhados: ["Músculos: abdômen"],
  imagem: "Flat Back.jpg",
  link: "1cI9DqtD9gI",
  video: "Flat Back.mp4"
},
{
  nome: "Spine C-Curve",
  descricao: "Posição: Sentado no Barrel com os pés fixados na escada. Execução: Desloque o tronco para trás realizando uma flexão (rolamento) da coluna e retorne ao neutro e depois faça uma extensão crescendo a coluna.",
  nivel: "Nível: Intermediário",
  local: "Local: Barrel",
  musculosTrabalhados: ["Músculos: abdômen", " mobilidade de coluna"],
  imagem: "Spine C-Curve.jpg",
  link: "g71ytayWFkY",
  video: "Spine C-Curve.mp4"
},
{
  nome: "Spine Wave",
  descricao: "Posição: Sentado no Barrel com os pés fixados na escada. Execução: Desloque o tronco para trás mantendo a coluna alinhada e retorne realizando uma flexão (rolamento) e ao final faça a correção por meio de uma extensão de coluna até alinhar ao centro.",
  nivel: "Nível: Intermediário",
  local: "Local: Barrel",
  musculosTrabalhados: ["Músculos: abdômen", " mobilidade de coluna"],
  imagem: "Spine Wave.jpg",
  link: "Z7nnn_whB0E",
  video: "Spine Wave.mp4"
},
{
  nome: "Kayak",
  descricao: "Posição: Sentado no Barrel com os pés fixados na escada. Execução: Desloque o tronco para trás realizando uma rotação movimentando os braços e retorne na posição inicial.",
  nivel: "Nível: Avançado",
  local: "Local: Barrel",
  musculosTrabalhados: ["Músculos: abdômen"],
  imagem: "Kayak.jpg",
  link: "E9NQkmLvbJs",
  video: "Kayak.mp4"
},
{
  nome: "Side Sit-Up",
  descricao: "Posição: Sentado de lado no Barrel com os pés fixados na escada e mãos na cabeça. Execução: Desloque o tronco realizando uma inclinação e retorne para a posição inicial terminando com um rotação de tronco.",
  nivel: "Nível: Avançado",
  local: "Local: Barrel",
  musculosTrabalhados: ["Músculos: abdômen"],
  imagem: "Side Sit-Up.jpg",
  link: "Fon-Vukf0-I",
  video: "Side Sit-Up.mp4"
},
{
  nome: "Side Strength",
  descricao: "Posição: Apoiado com o quadril de lado no Barrel, com os pés fixados na escada e os joelhos estendidios. Execução: Mantenha o tronco estável e realize o desafio com o movimento de abdução do braço de cima.",
  nivel: "Nível: Avançado",
  local: "Local: Barrel",
  musculosTrabalhados: ["Músculos: abdômen"],
  imagem: "Side Strength.jpg",
  link: "R6NSnds6Sm4",
  video: "Side Strength.mp4"
},
{
  nome: "Single Leg",
  descricao: "Posição: Sentado em cima do Barrel, de costas para a escada, com as mãos apoiadas para trás e os joelhos estendidios. Execução: Mantenha o tronco estável e realize o single leg, alternando o movimento das pernas.",
  nivel: "Nível: Avançado",
  local: "Local: Barrel",
  musculosTrabalhados: ["Músculos: abdômen", " flexores de Quadril", " quadríceps"],
  imagem: "Single Leg.jpg",
  link: "rrZjU7lReu0",
  video: "Single Leg.mp4"
},
{
  nome: "Teaser",
  descricao: "Posição: Sentado em cima do Barrel, de costas para a escada, com as mãos apoiadas para trás e os joelhos estendidios. Execução: Mantenha o tronco estável e realize o movimento de quadril, mantendo os joelhos estendidos.",
  nivel: "Nível: Avançado",
  local: "Local: Barrel",
  musculosTrabalhados: ["Músculos: abdômen", " flexores de Quadril", " quadríceps"],
  imagem: "Teaser.jpg",
  link: "wG22Qx-w8tk",
  video: "Teaser.mp4"
},
{
  nome: "Leg Circle",
  descricao: "Posição: Sentado em cima do Barrel, de costas para a escada, com as mãos apoiadas para trás e os joelhos estendidios. Execução: Mantenha o tronco estável e realize o movimento de rotação no quadril, mantendo os joelhos estendidos.",
  nivel: "Nível: Avançado",
  local: "Local: Barrel",
  musculosTrabalhados: ["Músculos: abdômen", " flexores de Quadril", " quadríceps"],
  imagem: "Leg Circle.jpg",
  link: "BwRAJ0270pI",
  video: "Leg Circle.mp4"
},
{
  nome: "Teaser Twist",
  descricao: "Posição: Sentado em cima do Barrel, de costas para a escada, com as mãos apoiadas para trás e os joelhos estendidios. Execução: Mantenha o tronco estável e realize o movimento de rotação troco levando a mão em direção ao joelho oposto. Retorne e alterne os lados.",
  nivel: "Nível: Avançado",
  local: "Local: Barrel",
  musculosTrabalhados: ["Músculos: abdômen", " flexores de Quadril", " quadríceps", " MMSS"],
  imagem: "Teaser Twist.jpg",
  link: "vxdNz9W9ErQ",
  video: "Teaser Twist.mp4"
},
{
  nome: "Bent Elbow",
  descricao: "Posição: Sentado no Barrel com os pés fixados na escada. Execução: Desloque o tronco para trás realizando uma flexão (rolamento) da coluna e retorne ao neutro e depois faça uma extensão de coluna com os braços e flexão de ombro.",
  nivel: "Nível: Intermediário",
  local: "Local: Barrel",
  musculosTrabalhados: ["Músculos: abdômen", " mobilidade de coluna"],
  imagem: "Bent Elbow.jpg",
  link: "hDrtITzscKI",
  video: "Bent Elbow.mp4"
},
{
  nome: "Side Bend",
  descricao: "Posição: Em pé de lado no Barrel com os pés fixados na escada e mãos segurando um bastão. Execução: Estabilize o tronco realizando uma inclinação juntamente com os MMSS.",
  nivel: "Nível: Intermediário",
  local: "Local: Barrel",
  musculosTrabalhados: ["Músculos: abdômen", " Quadrado Lombar", " MMSS"],
  imagem: "Side Bend.jpg",
  link: "p_Uomtw-QBw",
  video: "Side Bend.mp4"
},
{
  nome: "Straight Back",
  descricao: "Posição: Sentado no Barrel com os pés fixados na escada. Execução: Desloque o tronco para trás mantendo a coluna alinhada e movimente o braço.",
  nivel: "Nível: Avançado",
  local: "Local: Barrel",
  musculosTrabalhados: ["Músculos: abdômen"],
  imagem: "Straight Back.jpg",
  link: "8iEI1rlqSNU",
  video: "Straight Back.mp4"
},
{
  nome: "Shoulder Stretch",
  descricao: "Posição: Em pé na frente do Barrel com o tronco levemente deslocado para frente e as mãos apoiadas. Execução: Desloque o tronco para trás agachando e delocando o quadril para trás, mantendo as mãos aopiadas no barrel, alongando os ombros.",
  nivel: "Nível: Básico",
  local: "Local: Barrel",
  musculosTrabalhados: ["Músculos: mobilidade de ombro"],
  imagem: "Shoulder Stretch.jpg",
  link: "vqShIP1g9TM",
  video: "Shoulder Stretch.mp4"
},
{
  nome: "Shoulder Stretch with Rotation",
  descricao: "Posição: Em pé na frente do Barrel com o tronco levemente deslocado para frente e as mãos apoiadas. Execução: Desloque o tronco para trás agachando e delocando o quadril para trás, mantendo as mãos aopiadas no barrel, alongando os ombros. Abra um dos braços para a lateral e passe por baixo, realizando a rotação de tornco.",
  nivel: "Nível: Básico",
  local: "Local: Barrel",
  musculosTrabalhados: ["Músculos: mobilidade de ombro"],
  imagem: "Shoulder Stretch with Rotation.jpg",
  link: "ITClG4Rdfm0",
  video: "Shoulder Stretch with Rotation.mp4"
},
{
  nome: "Sit-Up",
  descricao: "Posição: Em pé de lado no Barrel com os pés fixados na escada e mãos posicionadas na nuca. Execução: Estabilize o tronco realizando uma inclinação de tronco.",
  nivel: "Nível: Intermediário",
  local: "Local: Barrel",
  musculosTrabalhados: ["Músculos: abdômen", " Quadrado Lombar"],
  imagem: "Sit-Up.jpg",
  link: "CVikOfXjsa4",
  video: "Sit-Up.mp4"
},
{
  nome: "Sit-Up Rotation",
  descricao: "Posição: Em pé de lado no Barrel com os pés fixados na escada e mãos posicionadas na nuca. Execução: Estabilize o tronco realizando uma inclinação de tronco, retorne a posicição inicial e realize uma rotação.",
  nivel: "Nível: Intermediário",
  local: "Local: Barrel",
  musculosTrabalhados: ["Músculos: abdômen", " Quadrado Lombar"],
  imagem: "Sit-Up Rotation.jpg",
  link: "BsWOaWfOqBs",
  video: "Sit-Up Rotation.mp4"
},


      // Barrel = 20









          
      
  // Sobre Joseph Pilates
           {
        nome: "Joseph Pilates",
        descricao: "O Método Pilates é um sistema de exercícios físicos criado por Joseph Pilates no início do século 20, focado na melhoria da flexibilidade, força muscular, controle postural e consciência corporal. Ele envolve movimentos controlados e precisos que visam fortalecer os músculos centrais (core), melhorar a postura e promover o equilíbrio entre o corpo e a mente. O Pilates pode ser praticado no solo, usando o peso do corpo, ou em aparelhos específicos, como o Reformer, Cadillac e Chair.",
        nivel: "",
        local: "",
        musculosTrabalhados: [""],
        imagem: "joseph_pilates.jpg",
        link: "GM1vqkIZYZo",
        video: "joseph_pilates.mp4"
              },
           {
        nome: "Joseph Pilates - Técnicas de Joseph Pilates",
        descricao: "Vídeo sobre as Técnicas de Joseph Pilates.",
        nivel: "",
        local: "",
        musculosTrabalhados: [""],
        imagem: "joseph_pilates.jpg",
        link: "oAKf77cbYu8",
        video: "joseph_pilates.mp4"
              },
           {
        nome: "Joseph Pilates - Tribute",
        descricao: "Vídeo sobre de Tribute a Joseph Pilates.",
        nivel: "",
        local: "",
        musculosTrabalhados: [""],
        imagem: "Tribute.jpg",
        link: "-DG52tULsO8",
        video: "Tribute.mp4"
              },
           {
        nome: "Joseph Pilates - Histórico parte 1",
        descricao: "Vídeo sobre o histórico de Joseph Pilates - parte 1.",
        nivel: "",
        local: "",
        musculosTrabalhados: [""],
        imagem: "Histórico.jpg",
        link: "SC8eh-bjhwY",
        video: "Histórico.mp4"
              },
           {
        nome: "Joseph Pilates - Histórico parte 2",
        descricao: "Vídeo sobre o histórico de Joseph Pilates - parte 2.",
        nivel: "",
        local: "",
        musculosTrabalhados: [""],
        imagem: "Histórico.jpg",
        link: "mIHc_fp4D8I",
        video: "Histórico.mp4"
              },
           {
        nome: "Joseph Pilates - Histórico parte 3",
        descricao: "Vídeo sobre o histórico de Joseph Pilates - parte 3.",
        nivel: "",
        local: "",
        musculosTrabalhados: [""],
        imagem: "Histórico.jpg",
        link: "TUzbhmnM2Pc",
        video: "Histórico.mp4"
              },
           {
        nome: "Joseph Pilates - Archival Footage",
        descricao: "Vídeo sobre arquivo de imagens de Joseph Pilates.",
        nivel: "",
        local: "",
        musculosTrabalhados: [""],
        imagem: "Archival Footage.jpg",
        link: "YeU_DiT_nhc",
        video: "Archival Footage.mp4"
              },
           {
        nome: "Joseph Pilates - Workouts",
        descricao: "Vídeo sobre Workouts com Joseph Pilates.",
        nivel: "",
        local: "",
        musculosTrabalhados: [""],
        imagem: "Workouts.jpg",
        link: "-wTNsF47HQs",
        video: "Workouts.mp4"
              },
           {
        nome: "Joseph Pilates - vídeos gerais",
        descricao: "Vídeo gerais com Joseph Pilates.",
        nivel: "",
        local: "",
        musculosTrabalhados: [""],
        imagem: "vídeos gerais.jpg",
        link: "iDHFLN7janc",
        video: "vídeos gerais.mp4"
              },
           {
        nome: "Joseph Pilates - no Reformer",
        descricao: "Vídeo de Joseph Pilates no Reformer.",
        nivel: "",
        local: "",
        musculosTrabalhados: [""],
        imagem: "no Reformer.jpg",
        link: "3_lAdlFmcVk",
        video: "no Reformer.mp4"
              },
           {
        nome: "Joseph Pilates - Principios del Reformer",
        descricao: "Vídeo sobre o Principios del Reformer.",
        nivel: "",
        local: "",
        musculosTrabalhados: [""],
        imagem: "Principios del Reformer.jpg",
        link: "mzVxfETQmMs",
        video: "Principios del Reformer.mp4"
              },
           {
        nome: "Joseph Pilates - Wunda Chair",
        descricao: "Vídeo sobre a Wunda Chair.",
        nivel: "",
        local: "",
        musculosTrabalhados: [""],
        imagem: "Wunda Chair.jpg",
        link: "2GjDtbvUvQs",
        video: "Wunda Chair.mp4"
              },
           {
        nome: "Joseph Pilates - aula no Cadillac",
        descricao: "Vídeo sobre aula de Joseph Pilates no Cadillac.",
        nivel: "",
        local: "",
        musculosTrabalhados: [""],
        imagem: "aula no Cadillac.jpg",
        link: "74-tNrb2Vpw",
        video: "aula no Cadillac.mp4"
              },
           {
        nome: "Joseph Pilates - Corrector Espinal",
        descricao: "Vídeo sobre Joseph Pilates no Corrector Espinal.",
        nivel: "",
        local: "",
        musculosTrabalhados: [""],
        imagem: "Corrector Espinal.jpg",
        link: "jvQGhubiPuE",
        video: "Corrector Espinal.mp4"
              },

              // Romana Kryzanowska
           {
        nome: "Romana Kryzanowska",
        descricao: "Vídeo sobre Romana Kryzanowska. Romana estudou balé quando criança. Aos 16 anos, enquanto estudava na Escola de Balé Americano de George Balanchine , ela sofreu uma lesão no tornozelo e foi levada por Balanchine a Joseph Pilates para ver se exercícios em vez de uma operação resolveriam o problema. Os exercícios foram um sucesso e ela continuou a estudar com Pilates. Romana Kryzanowska se tornou a protegida de Pilates.",
        nivel: "",
        local: "",
        musculosTrabalhados: [""],
        imagem: "Romana Kryzanowska.jpg",
        link: "YRX_KaPZLUk",
        video: "Romana Kryzanowska.mp4"
              },
           {
        nome: "Histórico Romana Kryzanowska - Parte 1",
        descricao: "Vídeo sobre o Histórico de Romana Kryzanowska - Parte 1.",
        nivel: "",
        local: "",
        musculosTrabalhados: [""],
        imagem: "Histórico Romana Kryzanowska - Parte 1.jpg",
        link: "0-_4mdmFiB4",
        video: "Histórico Romana Kryzanowska - Parte 1.mp4"
              },
           {
        nome: "Histórico Romana Kryzanowska - Parte 2",
        descricao: "Vídeo sobre o Histórico de Romana Kryzanowska - Parte 2.",
        nivel: "",
        local: "",
        musculosTrabalhados: [""],
        imagem: "Histórico Romana Kryzanowska - Parte 2.jpg",
        link: "yPyMf4dcczQ",
        video: "Histórico Romana Kryzanowska - Parte 2.mp4"
              },
           {
        nome: "Histórico Romana Kryzanowska - Parte 3",
        descricao: "Vídeo sobre o Histórico de Romana Kryzanowska - Parte 3.",
        nivel: "",
        local: "",
        musculosTrabalhados: [""],
        imagem: "Histórico Romana Kryzanowska - Parte 3.jpg",
        link: "MGdqxeqBl_Q",
        video: "Histórico Romana Kryzanowska - Parte 3.mp4"
              },









                 
        
];

